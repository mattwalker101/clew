# Deepening & Dependency Strategies
...
Dependencies that have local test stand-ins (PGLite/SQLite for Postgres, in-memory filesystem). Deepenable if the stand-in exists. The deepened module is tested with the stand-in running in the test suite. The seam is internal; no port at the module's external interface.
