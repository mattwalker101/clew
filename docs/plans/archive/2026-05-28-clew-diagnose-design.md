# Design Spec: Native clew-diagnose Operational Skill

## Goal
Implement a native, canonical `clew` operational skill `clew-diagnose` inside the `skills/` directory to enable automated, explainable, and local-first systematic debugging cycles using clew runbooks and verification gates.
...
skills/
  clew-diagnose/
    ...
    skill.md    # Strict debugging guidelines and reference
```

## Specification

### 1. Skill Manifest (`skills/clew-diagnose/clew.yaml`)
```yaml
id: clew-diagnose
version: 1.0.0
...
### 2. Instruction Guidelines (`skills/clew-diagnose/skill.md`)
Detail rules for reproducing defects with naming conventions, isolating minimal code blocks, developing structural hypotheses, red-to-green transitions, and git status pruning.
